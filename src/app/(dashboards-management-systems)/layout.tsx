...
export default function MyPageLayout({
  ...
  return (
    <main className={`${lato.className} flex flex-row w-full bg-slate-900`}>
      <section className="fixed z-20 shadow-md lg:block hidden min-h-screen">
        <Sidebar />
      </section>
      <section className="w-full lg:pl-[20px] min-h-screen flex flex-col ">
        <div className="lg:pl-14 lg:pr-14 p-1 w-full mt-2 fixed">
          <div className="p-5 border border-slate-50/10 rounded-md bg-slate-800 text-slate-50 z-30">Navigation</div>
        </div>
        <div className="mt-10 lg:pl-14 lg:pr-10 h-screen">{children}</div>
      </section>
    </main>
  );
}
